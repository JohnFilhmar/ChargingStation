...
void serials(){
  float batvolt = voltage(voltageSensorPin1);
  float solvolt = voltage(voltageSensorPin2);
  ...
  if(temp >= 40.0){
    ...
    digitalWrite(4,LOW); // TURN THE FAN OFF IF THE TEMPERATURE IS AVERAGE OR LOWs
  }
  if(temp >= 50){
    ...
    digitalWrite(2,LOW); // TURNS OFF THE INVERTER IF IT GETS WAYYYY TOO HOT THAT MAKES THE GLUE STICKS THAT HOLDS SOME PART TOGETHER MAY LOOSE VISCOSITY THUS MAKE IT FELL APART
  }
}
